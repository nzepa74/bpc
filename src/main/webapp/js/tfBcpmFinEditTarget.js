tfBcpmFinEditTarget = (function () {
    ...
    function _baseURL() {
        return 'api/tfBcpmFinEditTarget/';
    }
    ...
    function checkPermission(year, companyId, statusFlag) {
        ...
        $.ajax({
            url: 'api/tfBcpmFinList/checkPermission',
            type: 'GET',
    ...
    function getPreData() {
        ...
        $.ajax({
            url: 'api/tfBcpmFinAddTarget/getFinKpi',
            type: 'GET',
            success: function (res) {
                for (let i in res) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status === 1) {
                            ...
                            let targetId = $('#targetId').val();
                            let newUrlIS = window.location.origin + '/tfBcpmFinEditTarget?taId=' + targetAuditId + '&&tId=' + targetId
    ...
    function getAllocatedWt(year, companyId) {
        $.ajax({
            url: 'api/tfBcpmFinAddTarget/getAllocatedWt',
            type: "GET",
            data: {year: year, companyId: companyId},
            success: function (res) {
                if (res.status === 1) {
...
$(document).ready(
    function () {
        ...
        tfBcpmFinEditTarget.initEditor();
        tfBcpmFinEditTarget.getTarget();
        tfBcpmFinEditTarget.getPreData();
        tfBcpmFinEditTarget.autoSuggest();
        tfBcpmFinEditTarget.toggleLabel(year);
        tfBcpmFinEditTarget.actionBtnEdit();
        tfBcpmFinEditTarget.editTarget();
        tfBcpmFinEditTarget.getAllocatedWt(year, companyId);
        tfBcpmFinEditTarget.getAttachments();
        tfBcpmFinEditTarget.deleteFile();
        tfBcpmFinEditTarget.viewFile();
        tfBcpmFinEditTarget.downloadFile();
        tfBcpmFinEditTarget.remarkModalEditMode();
        tfBcpmFinEditTarget.remarkModalDisplayMode();
        tfBcpmFinEditTarget.addRemark();
        tfBcpmFinEditTarget.editRemark();
        tfBcpmFinEditTarget.addComment();
        tfBcpmFinEditTarget.getComment();
        tfBcpmFinEditTarget.closeOrReopenTarget();
        tfBcpmFinEditTarget.deleteTarget();
        tfBcpmFinEditTarget.getEditHistoryDetail();
        tfBcpmFinEditTarget.checkCommentPolicy();
        tfBcpmFinEditTarget.getCommentPolicy();
        tfBcpmFinEditTarget.btnGoBack();
    });
