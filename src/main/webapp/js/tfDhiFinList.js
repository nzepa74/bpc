tfDhiFinList = (function () {
    "use strict";
    let form = $('#targetListForm');
    let isSubmitted = false;

    function _baseURL() {
        return 'api/tfDhiFinList/';
    }
    ...
    function navigateToTfDhiFinAddTarget() {
        $('.btnAddNewTarget').on('click', function () {
            let year = $('#year').val();
            let companyId = $('#companyId').val();
            window.location.href = 'tfDhiFinAddTarget?yId=' + year + '&&cId=' + companyId
        });
    }
    ...
    function navigateToEditTarget() {
        $('#targetListTable tbody').on('click', 'tr .finKpi', function () {
            ...
            window.location.href = 'tfDhiFinEditTarget?taId=' + targetAuditId + '&&tId=' + targetId
        });
    }
    ...
    function getComment() {
        $('#targetListTable tbody').on('click', 'tr .btnViewComment', function () {
            ...
            $('.targetNameDisplay').empty().html('<strong>' + targetName + ' </strong>');
            $.ajax({
                url: 'api/tfDhiFinEditTarget/getFinComment',
                type: 'GET',
    ...
    function getAttachment() {
        $('#targetListTable tbody').on('click', 'tr .btnAttachment', function () {
            ...
            $.ajax({
                url: 'api/tfDhiFinEditTarget/getAttachments',
                type: "GET",
                data: {targetId: targetId},
                success: function (res) {
                    if (res.status === 1) {
    ...
    function viewFile() {
        $('.fileListDiv ul').on('click', 'li .viewFile', function () {
            ...
            let url = 'api/tfDhiFinEditTarget/viewFile/' + fileId;
            $.ajax({
                url: url,
                type: 'GET',
                data: {fileId: fileId},
    ...
    function downloadFile() {
        $('.fileListDiv ul').on('click', 'li span.fileName', function () {
            let fileId = $(this).closest('.fileUl .fileLi').find('.fileId').val();
            let url = 'api/tfDhiFinEditTarget/downloadFile/' + fileId;
            $.ajax({
                url: url,
                type: 'GET',
                data: {fileId: fileId},
    ...
    return {
        ...
        , navigateToTfDhiFinAddTarget: navigateToTfDhiFinAddTarget
        , btnSearch: btnSearch
...
$(document).ready(
    function () {
        let year = $('#year').val();
        let companyId = $('#companyId').val();
        tfDhiFinList.triggerSearch();
        tfDhiFinList.navigateToTfDhiFinAddTarget();
        tfDhiFinList.btnSearch();
        tfDhiFinList.navigateToEditTarget();
        tfDhiFinList.getWriteup();
        tfDhiFinList.getComment();
        tfDhiFinList.getAttachment();
        tfDhiFinList.viewFile();
        tfDhiFinList.downloadFile();
        tfDhiFinList.submit();
        tfDhiFinList.revert();
        tfDhiFinList.approve();
        tfDhiFinList.searchByStage();
        tfDhiFinList.btnGoBack();
    });
