tfBcpmOrgMgtAddTarget = (function () {
    "use strict";
    let form = $('#targetForm');
    let isSubmitted = false;

    function _baseURL() {
        return 'api/tfBcpmOrgMgtAddTarget/';
    }
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status === 1) {
                    ...
                    setTimeout(function () {
                        $('.headingWt').empty().text(orgManagementWt + '%');
                    }, 100);
                } else {
...
$(document).ready(
    function () {
        let year = $('#year').val();
        let companyId = $('#companyId').val();
        tfBcpmOrgMgtAddTarget.getPreData();
        tfBcpmOrgMgtAddTarget.autoSuggest();
        tfBcpmOrgMgtAddTarget.initEditor();
        tfBcpmOrgMgtAddTarget.initialSetup();
        tfBcpmOrgMgtAddTarget.onChangeYearAndCompany();
        tfBcpmOrgMgtAddTarget.toggleLabel(year);
        tfBcpmOrgMgtAddTarget.addTarget();
        tfBcpmOrgMgtAddTarget.getAllocatedWt(year, companyId);
        tfBcpmOrgMgtAddTarget.btnAddMore();
        tfBcpmOrgMgtAddTarget.deleteSubTarget();
        tfBcpmOrgMgtAddTarget.btnGoBack();
    });
